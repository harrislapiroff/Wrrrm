...
Crafty.c "SnakePart",
	...
	snakepart: (@snake, @circumference_location, @altitude=0) ->
		...
		@attr {rotation: @circumference_location*360/@snake.circumference(), y: @snake.pos()._y-@altitude, x: @snake.pos()._x+(@snake.pos()._w+@pos()._w)/2}
		
		# set the origin to the center of the snake
		@origin @pos()._w/2, (@snake.pos()._h)/2+@altitude
